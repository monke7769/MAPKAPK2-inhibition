...
import matplotlib.pyplot as plt
from rdkit import Chem
from rdkit.Chem import rdFingerprintGenerator, rdMolDescriptors, AllChem, Draw, rdDepictor, rdCoordGen
from sklearn.neural_network import MLPClassifier
...
def smilesAvalon(smiles):
    mol = Chem.MolFromSmiles(smiles)
    if mol:
        fp = GetAvalonFP(mol) # fp=fingerprint
        return list(fp)


data_p = data[data['Active']==1].iloc[:840]
data_n = data[data['Active']==0].iloc[:2110]

data_trunc = pd.concat([data_p,data_n])[['Name','smiles','Active']].sample(frac=1, random_state=42).reset_index()

ava_fp = []
for mol in data_trunc['smiles']:
    desired_chem = Chem.MolFromSmiles(mol) # get the molecule from smiles representation
    fp = GetAvalonFP(desired_chem)
    fp_array = np.frombuffer(fp.ToBitString().encode(), 'u1') - ord('0')
    ava_fp.append(fp_array)

X = pd.DataFrame(ava_fp)
y = data_trunc['Active']
...
mfp = []
for mol in acd990['Smiles']:
    desired_chem = Chem.MolFromSmiles(mol) # get the molecule from smiles representation
    fp = GetAvalonFP(desired_chem)
    fp_array = np.frombuffer(fp.ToBitString().encode(), 'u1') - ord('0')
    mfp.append(fp_array)

X_acd = pd.DataFrame(mfp)
y_acd = acd990['Active']
...
for size in layersizes: # training/validation, 0.1 test, ACD for each network architecture
  nnet = Sequential()

  # Add first hidden layer with ReLU activation and dropout
  nnet.add(Dense(size[0], activation='relu', input_shape=(512,))) # Replace input_dim with the number of input features (different for each featurizer)
  ...
  nnet.add(Dense(size[0], activation='relu', input_shape=(512,)))
  nnet.add(Dropout(0.25))  # Dropout rate of 0.25
  for i in size[1:]:
    nnet.add(Dense(i, activation='relu'))
    nnet.add(Dropout(0.25))
  ...
  nnet.compile(optimizer=Adam(),
                ...
                metrics=['accuracy'])
  nnet.fit(X, y.values.ravel(), validation_split=0.2, epochs=100, verbose=1, callbacks=[early_stopping])
  ...
  y_pred_acd = np.round(nnet.predict(X_acd))
  pos.append(np.sum(y_pred_acd == 1))  # Count of positives (FP)
  neg.append(np.sum(y_pred_acd == 0))  # Count of negatives (TN)
